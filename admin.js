...
async function fetchProducts(){
  const { data, error } = await supabase.from('products').select('id,name,price,description,image_url,category_id, categories!inner(name)').order('id',{ascending:true});
...
async function repaintProducts(){
  // join categories via select above already fetched
  const { data, error } = await supabase.from('products')
    .select('id,name,price,category_id, description, image_url, categories(name)')
...
export const AdminActions = {
  ...
  async saveProduct(){
    ...
    const payload = { name, price, category_id, description, image_url };
